...
//const Grid = 
  ...
  wouldExitBounds(xMove, yMove) {
    ...
    for (let i = 0; i < this.bounds.length; i++) {
        if (this.bounds[i].x + xMove > this.totalWidth() || this.bounds[i].x + xMove < 0) return true;
        if (this.bounds[i].y + yMove > this.totalHeight() || this.bounds[i].y + yMove < 0) return true;
    }
    return false;
  }
  update() {
