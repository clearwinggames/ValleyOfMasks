...
//const Grid = 
  ...
  wouldExitBounds(xMove, yMove) {
    ...
    {   // somewhere in here it should be based on the location of the grid (x, y) relative to canvas bounds...
        if (this.bounds[i].x + this.bounds[i].w + xMove > this.bounds[i].canvas.w || 
            this.bounds[i].x + xMove < 0) 
          return true;
        if (this.bounds[i].y + this.bounds[i].h + yMove > this.bounds[i].canvas.h || 
            this.bounds[i].y + yMove < 0) 
          return true;
    }
    return false;
  }
