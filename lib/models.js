...
//const Grid = 
  ...
  turnBoundsOn(canvasElement) {
    ...
    bounds = [ { 'x': 0, 'y': 0, 'w': this.totalWidth(), 'h': this.totalHeight(), 'canvas': { 'w': xRes, 'h': yRes  } } ];
    this.bounds = bounds;
  ...
  wouldExitBounds(xMove, yMove) {
    ...
    for (let i = 0; i < this.bounds.length; i++) {
        if (this.bounds[i].x + this.bounds[i].w + xMove > this.totalWidth() || this.bounds[i].x + xMove < 0) return true;
        if (this.bounds[i].y + this.bounds[i].h + yMove > this.totalHeight() || this.bounds[i].y + yMove < 0) return true;
    }
    return false;
  }
  update() {
