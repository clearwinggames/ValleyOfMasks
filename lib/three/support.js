...
function startGameLoop(controlScheme, runAtBeginning, runInTheMiddle, runAtEnd) {
	...
	{ 
		...
		if (millisecondRate > 0) {
			advanceTimeOfDay(millisecondRate)
			// update background
			setSkyColorToString(timeOfDayToColor(timeOfDayHours, timeOfDayMinutes));
		}
		...
		if (useDefault3dControls == true) {
			if (upkey() || keyval('w')) {
				if (document.fullscreenElement !== null)
					...
					fullScreenBody();
					//expiring_top_text('This game is meant to be played in FullScreen, press the Q key to activate fullscreen and escape to exit.', 3500);
			}
			if (downkey() || keyval('s')) {
				collideWith = moveForwardWithCollision(camera, scene_data, -0.05 * (keyval('r') ? 2 : 1));
			}
...
function setSkyColorTo(newColor) {
//	threeScenes[0].Object.background = new THREE.Color(0x007f7f)
	threeScenes[0].Object.background = newColor;
}
function setSkyColorToString(newColorString) {
    threeScenes[0].Object.background = new THREE.Color(newColorString);
	//threeScenes[0].Object.background = newColor;
}
function lowerResolutionTo(ratio) {
    threeScenes[0].Object.renderer.setPixelRatio(ratio);
}
function adjustResolution(xFactor, yFactor) {
    threeScenes[0].Object.renderer.setSize(window.innerWidth / xFactor, window.innerHeight / yFactor, false);
}
function getCamera() {
