...
var timeOfDayMinutes = 0;
var millisecondRate = 0;
var msPerMinute = 1000 * 60;
var timeOfDayMs = 0;
var daysPassed = 0;

var keydownActions = [
	//{ name: 'delete_me', keyval: 'x', action: () => { deleteKeydownAction('delete_me'); } }
];

var activeIFrame = {};

let contentLibPath = '/lib/content/';

function deleteKeydownAction(nameOf) {
    keydownActions = keydownActions.filter((ent) => ent.name != nameOf);
}
function executeKeydownActions() {
   for (let i = 0; i < keydownActions.length; i++) {
		if (keyval(keydownActions[i].keyval) == true) {
			keydownActions[i].action(keydownActions[i]);
		}
   }
}
...
function startDayCycle(speedOfTime, durationOfMinute) {
	if (typeof speedOfTime == 'undefined') speedOfTime = 1;
	if (typeof durationOfMinute == 'undefined') durationOfMinute = 1000 * 6;
	millisecondRate = speedOfTime; 
	msPerMinute = durationOfMinute;
}
function advanceTimeOfDay(rate) {
   for (let i = 0; i < rate; i++) 
   {
	   timeOfDayMs += millisecondRate;
	   if (timeOfDayMs >= msPerMinute) 
	   {
		 timeOfDayMs = 0;
		 timeOfDayMinutes += 1;
...
/*function moveWithinSceneBy(xAmt, yAmt) {
	// when "turnCollisionOn" was called, an object was provided. (collidesWith, technically an array)
	if (typeof activeScene[i].supportCollision != 'undefined' && activeScene[i].supportCollision == true) {
		if (activeScene[i].wouldCauseCollision(
		for (let i = 0; i < activeScene.collidesWith.length; i++) {
...
function moveCurrentSceneBy(xAmt, yAmt) 
{
	for (let i = 0; i < activeScene.length; i++) 
	{
		if (typeof activeScene[i].supportCollision != 'undefined' && activeScene[i].supportCollision == true) 
		{
			console.log('activeScene[' + i.toString() + '] supports collision');
			// would collision happen?  if so, return
			if (activeScene[i].wouldCauseCollision(xAmt, yAmt)) return;
			if (activeScene[i].isOverlapping()) return;
			
			console.log('But it isnt colliding with anything');
		}
		if (typeof activeScene[i].supportBounds != 'undefined' && activeScene[i].supportBounds == true) 
		{
			// would a bounds-exit happen?  if so, potentially move the central collision object (often the player)
			if (activeScene[i].wouldExitBounds(xAmt, yAmt)) 
			{				
				for (let j = 0; j < activeScene.length; j++) 
				{
					if (typeof activeScene[j].collidesWith != 'undefined') 
					{
						for (let h = 0; h < activeScene[j].collidesWith.length; h++) 
						{
							activeScene[j].collidesWith[h].x += -xAmt;
							activeScene[j].collidesWith[h].y += -yAmt;
						}
					}
				}
				return;
	...
	for (let i = 0; i < activeScene.length; i++) 
	{
		if (activeScene[i].constructor.name == 'Grid') // todo: replace this with "is this a collideable" logic
		{
...
function esckey() { return keyval('escape'); }
...
function timeOfDayToColor(timeOfDayHours, timeOfDayMinutes)
{//"#abcdef"
    let colors = [
		'#00002f', // night black 
		'#007fff', // day blue 
	...
	let minutes = timeOfDayHours * 60;
	minutes += timeOfDayMinutes;
	if (minutes >= 360 && minutes <= 1080)
	{
		return colors[1];
	}
	else if ((minutes >= 330 && minutes <= 337))
	{
		return colors[2];
	}
	else if ((minutes >= 338 && minutes <= 341) || (minutes <= 1086 && minutes >= 1081)) {
		return colors[3];
	}
	else if ((minutes >= 342 && minutes <= 347) || (minutes <= 1092 && minutes >= 1086)) {
		return colors[5];
	}
	else if ((minutes >= 348 && minutes < 360) || (minutes <= 1098 && minutes >= 1092)) {
		return colors[6];
	}
	return colors[0]; 
}
...
function show_fullscreen_mid_text(iframeSrc, iframeName, divName, caption, keypress, height) {

	if (typeof height == 'undefined') height = '400px';

	if (document.getElementById(iframeName) != null) return;

	update_mid_text('');

	keydownActions.push({ name: 'temporary_' + iframeName, keyval: keypress, action: (parent) => 
	{
		//alert('Hit action! ' + parent.name + '; ' + iframeName + '; ' + divName);	

		if (document.getElementById(iframeName) != null) 
		{
			activeIFrame = document.getElementById(iframeName);
			
			let theDiv = document.getElementById(iframeName).contentDocument.getElementById(divName);
			//if (theDiv == null) alert('null');
			//if (theDiv != null) alert('not null');
			...
			theDiv.focus();
			//document.getElementById(iframeName).contentDocument.getElementById(divName).requestFullscreen();
			//document.getElementById(iframeName).contentDocument.getElementById(divName).focus();	
		}
		deleteKeydownAction(parent.name);//'temporary_' + iframeName);
	}});
...
window.addEventListener('keydown', (event) => {
     if (event.key == 'q') {
		 ...
		 window.parent.fullScreenBody();
		 window.parent.update_mid_text('');
		 window.parent.hide_mid_text();
	 }
	 else {
	...
	setTimeout(() => {
		scriptIntoIframe(iframe, sendScript);
	}, 1000);
}
...
function update_mid_text(text) {
	if (text.length == 0) hide_mid_text();
...
function clear_all_mid_text() {
	update_mid_text(''); // same thing as below?
   //document.getElementById('midText').innerHTML = '';
}

function fullScreenDiv(divName) {
...
function fullScreenBody() {
	let theDiv = document.body;
	
			theDiv.style.width = '85vw';
			theDiv.style.height = '85vh';
			theDiv.style.margin = '0';
			theDiv.style.padding = '0';
			theDiv.style.boxSizing = 'border-box';
			theDiv.style.backgroundColor = 'white';
...
function fullScreenCanvas() {
	let theDiv = document.getElementsByTagName('canvas')[0];
...
function scriptIntoIframe(iframe, script){
    var head = iframe.contentWindow.document.getElementsByTagName('head')[0];
    var scriptElement = iframe.contentWindow.document.createElement('script');
