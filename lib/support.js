...
function drawImage(x, y, imagePath) {
	...
	if (location.origin.indexOf('file') == 0) {
		image.src = location.pathname.replace('Default.html', '') + '/JSLib/ContentLib/' + imagePath;
	}
	else {
		image.src = location.origin + '/js/cibrgames/CIBRGamesJS/JSLib/ContentLib/' + imagePath;
	}
