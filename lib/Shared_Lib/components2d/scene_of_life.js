function scene_of_life() {
    ...
    let renderSprite = new Sprite(64, 96, 32, 32, 'default.png');

    renderSprite.turnOnGravityRelative(0.25, 4);
    ...
    let secondGrid =  new Grid(64, 96, 32, 32, 8, 8, waterSprite);
    firstGrid.turnBoundsOn(canvas);
    secondGrid.turnCollisionOn([ renderSprite ], (grid, cw, tile) => { console.log('Works: ' + cw.x.toString()); });

    keydownActions.push({ name: 'five', keyval: 'j', action: () => { renderSprite.momentumY = -6; } });
